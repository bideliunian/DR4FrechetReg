...
for (i in 1:times_repeat) {
  load(file = paste(save_path, "/model12.rho=", rho,"nonellip=", non_elliptical,
                    ".result",i,".RData",sep=""))
  # estimation error
...
# save(result_12, file='dist.model12.summary.Rdata')
...
result_34_b <- cbind(grid_34, error_mean_34, error_sd_34, order_perc_34)


############ function to write result in latex
result_34 <- result_34_b[order(result_34_b$model),]
result_mean <- round(as.matrix(result_34[,7:14]),3)
result_sd <- round(as.matrix(result_34[,15:22]),3)
result_prop <- round(as.matrix(result_34[,23:ncol(result_12)]), 2)
result_all <- matrix(0, nrow = 3*nrow(result_34), ncol = ncol(result_mean)+1)
...
ggplot(error_model1_stack, aes(x = ind, y = values, fill=p)) + 
  geom_boxplot()+labs(title="",x="methods", y = "errors") + theme(text = element_text(size = 20))  
